...
This Thinkful Full-stack Node Capstone project is a story of analysis and repair. The Sustainable NanoTechology Organization (SNO) has a production site (susnano.org) that has no back-end. It was thus a great candidate for demonstrating my newly acquired node.js skills, i.e., the use of endpoints for delivering static assets and RESTful (REpresentational State Transfer) API endpoints. It also entailed redesigning and building a client that makes use of the API.

In addition to enabling persistence of membership information for this professional society, I used this MVP to demonstrate how a new online service can be offered to members through a News database. The current SNO website (susnano.org) not only lacks server-side functionality, though. It is not wholly compliant with current software engineering practice in terms of mobile-first design, responsiveness and accessibility (a11y). The SNO CAPSTONE app was designed and built mobile-first, with responsiveness and a11y compliance in mind. 

Finally, SNO has no policy for privacy assurances for information collected. I added some language for a Policy Statement that will hopefully inform the SNO leadership of the need to design a privacy policy, post it on their website and keep current as privacy policy is in a high state of flux. Specifically, SNO needs to be aware of and address General Data Protection Regulations, especially as it is an international organization.

(Note: use of the SNO logo exclusively for this project was granted by the SNO Executive Director and Founder, Barbara Karn. The existing website membership form guided the creation of the form for this project, but no code from SNO was used.)
